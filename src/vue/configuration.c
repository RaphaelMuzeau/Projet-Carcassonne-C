...
enum Page page_configuration(Jeu *jeu, enum Configuration config) {
  ...
  while (prochaine_page == P_CONF_CUSTOM) {
    ...
    if (update_bouton(&confirmer) && champ_nb_joueur.saisie.len != 0 &&
        ...
        if (nb_joueur != 0 && nb_meeple != 0 && nb_tuiles != 0) {
            if (config == C_CSV) { // la pile a deja été chargée
                detruire_listejoueurs(jeu->joueurs);
                jeu->joueurs = creer_listejoueurs(nb_joueur, nb_meeple);
            } else {
                detruire_jeu(*jeu);
            ...
            if (prochaine_page != P_CONF_CUSTOM)
                continue;
        }
    }
